...
/**
 * Name of the GPS Measurements corrections interface.
 */
#define GNSS_MEASUREMENT_CORRECTIONS_INTERFACE   "gnss_measurement_corrections"

/**
 * Name of the GPS Visibility Control interface.
 */
#define GNSS_VISIBILITY_CONTROL_INTERFACE   "gnss_visibility_control"
...
/*
//Same with GnssLocation in HIDL
typedef struct {
    GpsLocation     location;
    ElapsedRealtime elapsedTime;
} GpsLocationEx;
*/
...
/*
* The gnss measure correction capability.
*/
typedef void (*gnss_set_capabilities_callback) (int capabilities);

typedef struct {
    /** set to sizeof(GnssMeasurementCorrectionCallbacks) */
    size_t size;
    gnss_set_capabilities_callback mea_correction_capability_callback;
} GnssMeasurementCorrectionCallbacks;
...
typedef struct
{
 double lat;                /** Represents latitude of the reflecting plane in degrees. */
 double lon;                /** Represents longitude of the reflecting plane in degrees. */
 double altitude;           /** Represents altitude of the reflecting point in the plane in meters above the WGS 84 reference ellipsoid.*/
 double azimuth;           	/** Represents azimuth clockwise from north of the reflecting plane in degrees.*/
}ReflectingPlane;//4

typedef struct
{
 uint16_t flag;//0x1:HAS_SAT_IS_LOS_PROBABILITY;0x2:HAS_EXCESS_PATH_LENGTH;0x4:HAS_EXCESS_PATH_LENGTH_UNC;0x8:HAS_REFLECTING_PLANE
 uint8_t constellation;
 uint16_t svid;
 float carrierFrequencyHz;
 float probSatIsLos;     /** The probability that the satellite is estimated to be in Line-of-Sight condition at the given location.*/
 float excessPathLengthMeters;  /** Excess path length to be subtracted from pseudorange before using it in calculating location.*/
 float excessPathLengthUncertaintyMeters;/** Error estimate (1-sigma) for the Excess path length estimate */
 ReflectingPlane reflectingPlane; /** Defines the reflecting plane characteristics such as location and azimuth.*/
}SingleSatCorrection_t;//55bytes

typedef struct
{
  double lat;               /** Represents latitude in degrees at which the corrections are computed. */
  double lon;               /** Represents longitude in degrees at which the corrections are computed. */
  double altitude;          /** Represents altitude in meters above the WGS 84 reference ellipsoid at which the corrections are computed.*/
  double horizontalUnc;     /** Represents the horizontal uncertainty (68% confidence) in meters on the device position at which the corrections are provided.*/
  double verticalUnc;       /** Represents the vertical uncertainty (68% confidence) in meters on the device position at which the corrections are provided.*/
  uint64_t toaGpsNanosecondsOfWeek;  /** Time Of Applicability, GPS time of week in nanoseconds. */
  SingleSatCorrection_t* satCorrections; //[TOTAL_CH_NUM]; /** A set of SingleSatCorrection each containing measurement corrections for a satellite in view.*/
}GnssMeasurementCorrections_t;
...
typedef struct {
    size_t size;
    int (*init) (GnssMeasurementCorrectionCallbacks* callbacks);
    int (*setCorrections) (GnssMeasurementCorrections_t* corrections, int single_count );
} GpsMeasureCorrectionsInterface;
...
typedef void (*gnss_nfw_notify_callback) (GnssNfwNotification* notification);
typedef bool (*isInEmergencySession_callback) ();

typedef struct {
    /** set to sizeof(GnssVisibilityControlCallbacks) */
    size_t size;
    gnss_nfw_notify_callback nfw_notify_callback;
    isInEmergencySession_callback is_in_emergency_session_calback;
} GnssVisibilityControlCallbacks;

typedef struct {
    size_t size;
    int (*init) (GnssVisibilityControlCallbacks* callbacks);
    int (*enableNfwLocationAccess) (const char* proxy_apps);
} GpsVisibilityControlInterface;
...
typedef struct {
	...
	/** bind network handle */
	void (*network_handle_bind) (long long int handle_val);
