#define LOG_TAG "GnssHAL_AGnssInterfaceExt"
#include "GnssExt.h"
#include <GnssUtils.h>
#include <errno.h>
...
std::vector<std::unique_ptr<ThreadFuncArgs>> GnssExt::sThreadFuncArgsList;
sp<IGnssCallback> GnssExt::sGnssCbIface = nullptr;
bool GnssExt::sInterfaceExists = false;
bool GnssExt::sWakelockHeldGnss = false;
bool GnssExt::sWakelockHeldFused = false;

GpsCallbacks GnssExt::sGnssCb = {
...
uint32_t GnssExt::sCapabilitiesCached = 0;
uint16_t GnssExt::sYearOfHwCached = 0;

GnssExt::GnssExt(gps_device_t* gnssDevice) :
        mDeathRecipient(new GnssHidlDeathRecipient(this)) {
    /* Error out if an instance of the interface already exists. */
    LOG_ALWAYS_FATAL_IF(sInterfaceExists);
    sInterfaceExists = true;
	ALOGE("%s: this is vendor ext HIDL_harry", __func__);
...
void GnssExt::locationCb(GpsLocation* location) {
...
void GnssExt::statusCb(GpsStatus* gnssStatus) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
...
void GnssExt::gnssSvStatusCb(GnssSvStatus* status) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
 ...
 */
void GnssExt::gpsSvStatusCb(GpsSvStatus* svInfo) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
...
void GnssExt::nmeaCb(GpsUtcTime timestamp, const char* nmea, int length) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
...
void GnssExt::setCapabilitiesCb(uint32_t capabilities) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
...
void GnssExt::acquireWakelockCb() {
    acquireWakelockGnss();
}

void GnssExt::releaseWakelockCb() {
    releaseWakelockGnss();
}


void GnssExt::acquireWakelockGnss() {
    sWakelockHeldGnss = true;
    updateWakelock();
}

void GnssExt::releaseWakelockGnss() {
    sWakelockHeldGnss = false;
    updateWakelock();
}

void GnssExt::acquireWakelockFused() {
    sWakelockHeldFused = true;
    updateWakelock();
}

void GnssExt::releaseWakelockFused() {
    sWakelockHeldFused = false;
    updateWakelock();
}

void GnssExt::updateWakelock() {
...
pthread_t GnssExt::createThreadCb(const char* name, void (*start)(void*), void* arg) {
    return createPthread(name, start, arg, &sThreadFuncArgsList);
}

void GnssExt::setSystemInfoCb(const LegacyGnssSystemInfo* info) {
    if (sGnssCbIface == nullptr) {
        ALOGE("%s: GNSS Callback Interface configured incorrectly", __func__);
        return;
    }
...
Return<bool> GnssExt::setCallback(const sp<IGnssCallback>& callback)  {
...
Return<bool> GnssExt::injectLocation(double latitudeDegrees,
...
Return<bool> GnssExt::injectTime(int64_t timeMs, int64_t timeReferenceMs,
...
Return<void> GnssExt::deleteAidingData(IGnss::GnssAidingData aidingDataFlags)  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
    } else {
...
Return<bool> GnssExt::setPositionMode(IGnss::GnssPositionMode mode,
...
Return<sp<::android::hardware::gnss::V1_0::IAGnssRil>> GnssExt::getExtensionAGnssRil() {
	
	return ::android::sp<::android::hardware::gnss::V1_0::IAGnssRil> {};
    	}
Return<sp<IAGnssRilExt>> GnssExt::getExtensionAGnssRilExt()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
    ...
    if (mGnssRil == nullptr) {
        ...
        if (agpsRilIface == nullptr) {
            ...
            mGnssRil = new AGnssRilExt(agpsRilIface);
        }
    }
    return mGnssRil;
}

Return<sp<IGnssConfiguration>> GnssExt::getExtensionGnssConfiguration()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssGeofencing>> GnssExt::getExtensionGnssGeofencing()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IAGnss>> GnssExt::getExtensionAGnss()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssNi>> GnssExt::getExtensionGnssNi()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssMeasurement>> GnssExt::getExtensionGnssMeasurement() {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssNavigationMessage>> GnssExt::getExtensionGnssNavigationMessage() {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssXtra>> GnssExt::getExtensionXtra()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssDebug>> GnssExt::getExtensionGnssDebug()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
Return<sp<IGnssBatching>> GnssExt::getExtensionGnssBatching()  {
    if (mGnssIface == nullptr) {
        ALOGE("%s: Gnss interface is unavailable", __func__);
        return nullptr;
    }
...
IGnssExt* HIDL_FETCH_IGnssExt(const char* /* hal */) {
    hw_module_t* module;
    IGnssExt* iface = nullptr;
    ...
    if (err == 0) {
        ...
        if (err == 0) {
            iface = new GnssExt(reinterpret_cast<gps_device_t*>(device));
        } else {
